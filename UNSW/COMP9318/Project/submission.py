## Yunqiu Xu
## z5096489
## Using Tree on Vowel2, Using KNN on Vowel3, Vowel4
## Based on P400 KNN
...
################### TR ##########################
def get_structure_and_vowels_TR(removed_stress, vowel):
    structure = ''
    num_vowels = 0
    count = 0
    vowels = []
...
def get_splitted_data(raw_data, vowel, consonant):
    ...
    for curr_row in raw_data[:]:
        ...
        if num_vowel <= 1:
        ...
        elif num_vowel == 2: ## Use TR here
            row_data = [target, word, ' '.join(removed_stress)]
            vowels, vowel_positions, num_vowels, structure = get_structure_and_vowels_TR(removed_stress, vowel)

            row_data.extend(vowels) # VOWEL1, VOWEL2
            row_data.extend(vowel_positions) # V1_IN_RS, V2_IN_RS
        
            prefix_suffix = get_prefix_and_suffix(vowel_positions, removed_stress, consonant)

            row_data.extend(prefix_suffix) # V1P, V1S, V2P, V2S
            ...
            row_data.append(structure_index) # STRUCTURE
...
def train(data, classifier_file):
    ...
    ################ df2 TR #################
    df2 = get_df_TR(vowel2)
    
    X2 = df2[['Vowel1', 'Vowel2','V1_in_RS', 'V2_in_RS', 'V1P1','V1S1', 'V2P1','V2S1']]
    y2 = df2[['Target']]
...
def test(data, classifier_file):
    ...
    for curr_row in data[:]:
        ...
        if num_vowel <= 1:
            ...
            df2 = get_df_TR([row_data])
            features = ['Vowel1', 'Vowel2','V1_in_RS', 'V2_in_RS', 'V1P1','V1S1', 'V2P1','V2S1']
            curr_pred = tree2.predict(df2[features])[0]
